...
uint8_t blSendCmdSetAddr(uint8_t addrHi, uint8_t addrLo) {
	...
	uint8_t sCmd[] = {CMD_SET_ADDRESS, 0x00, addrHi, addrLo};
	uint8_t rxBuf[50] = {0};
	SendESC(sCmd, 4);
	delayWhileRead(5);
...
uint8_t blSendCmdSetBuf(uint8_t len, uint8_t buf[256]) {
	uint8_t sCmd[] = {CMD_SET_BUFFER, 0x00, len == 0, len};
	uint8_t rxBuf[50] = {0};
	SendESC(sCmd, 4);
	delayWhileRead(5);
	uint16_t rxSize = GetESC(rxBuf, 20);
	if (rxSize && rxBuf[rxSize - 1] != brNONE) return 0;
...
uint8_t blVerifyFlash(uint8_t len, uint8_t buf[256], uint8_t addrHi, uint8_t addrLo) {
	if (blSendCmdSetAddr(addrHi, addrLo)) {
		...
		delayWhileRead(5);
		uint16_t rxSize = GetESC(rxBuf, 20);
		return rxSize ? rxBuf[rxSize - 1] : brNONE;
	}
	return 0;
}
...
uint16_t Check_4Way(uint8_t buf[]) {
	...
	if (cmd == cmd_DeviceInitFlash) {
		...
		O_Param_Len = 0x01;
		if (param < escCount) { // ESC Count
			if (currentPin != passthroughPins[param])
				changePin(passthroughPins[param]);
			if (Enable4Way) {
				buf[6] = ACK_OK; // ACK
		...
		O_Param_Len = 1;
		switch (blVerifyFlash(I_Param_Len, ParamBuf, addr_high, addr_low)) {
		case brSUCCESS:
			ack_out = ACK_OK;
			break;
		case brERRORVERIFY:
