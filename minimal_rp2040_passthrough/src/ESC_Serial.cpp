...
void pioResetESC() {
	gpio_init(currentPin);
	gpio_set_dir(currentPin, GPIO_OUT);
	gpio_put(currentPin, 0);
	delay(300);
	gpio_put(currentPin, 1);
	gpio_set_dir(currentPin, GPIO_IN);
	gpio_set_function(currentPin, escPassthroughPio == pio0 ? GPIO_FUNC_PIO0 : GPIO_FUNC_PIO1);
}
...
void delayMicrosWhileRead(uint32_t us) {
	elapsedMicros x = 0;
	do {
		while (pio_sm_get_rx_fifo_level(escPassthroughPio, escPassthroughSm)) {
